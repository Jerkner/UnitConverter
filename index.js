...
convertBtn.addEventListener("click", function() {
    ...
    length1 = Math.round(value * 3.281 * 10) / 10
    length2 = Math.round(value / 3.281 * 10) / 10
    lengthOut.innerHTML = `${length} meters = ${length1} feet
    <br> 
    ${length} feet = ${length2} meters`

    volume = Math.round(value * 10) / 10
    volume1 = Math.round(value * 0.264 * 10) / 10
    volume2 = Math.round(value / 0.264 * 10) / 10
    volumeOut.innerHTML = `${volume} liters = ${volume1} gallons
    <br> 
    ${volume} gallons = ${volume2} liters`

    mass = Math.round(value * 10) / 10
    mass1 = Math.round(value * 2.204 * 10) / 10
    mass2 = Math.round(value / 2.204 * 10) / 10
